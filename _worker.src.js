    ...
    async fetch(request, env, ctx) {
        try {
            ...
            if (upgradeHeader === 'websocket') {
                return await amclubsOverWSHandler(request);
            }
            ...
            switch (url.pathname.toLowerCase()) {
                ...
                case `/${fakeUserID}`: {
                    ...
                    const fakeConfig = await getamclubsConfig(userID, host, 'CF-FAKE-UA', url);
                ...
                case `/${userID}`: {
                    ...
                    const amclubsConfig = await getamclubsConfig(userID, host, userAgent, url);
                    ...
                    return new Response(amclubsConfig, {
                        status: 200,
 ...
 */
async function getamclubsConfig(userID, host, userAgent, _url) {
    // console.log(`------------getamclubsConfig------------------`);
    // console.log(`userID: ${userID} \n host: ${host} \n userAgent: ${userAgent} \n _url: ${_url}`);
...
/** -------------------processing logic-------------------------------- */
/**
 * Handles amclubs over WebSocket requests by creating a WebSocket pair, accepting the WebSocket connection, and processing the amclubs header.
 * @param {import("@cloudflare/workers-types").Request} request The incoming request object.
 * @returns {Promise<Response>} A Promise that resolves to a WebSocket response object.
 */
async function amclubsOverWSHandler(request) {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            const {
                ...
                addressRemote = '',
                rawDataIndex,
                amclubsVersion = new Uint8Array([0, 0]),
                isUDP,
                addressType,
            } = processamclubsHeader(chunk, userID);
            ...
            const amclubsResponseHeader = new Uint8Array([amclubsVersion[0], 0]);
            const rawClientData = chunk.slice(rawDataIndex);

            if (isDns) {
                const { write } = await handleUDPOutBound(webSocket, amclubsResponseHeader, log);
                udpStreamWrite = write;
                udpStreamWrite(rawClientData);
                return;
            }
            log(`processamclubsHeader-->${addressType} Processing TCP outbound connection ${addressRemote}:${portRemote}`);

            handleTCPOutBound(remoteSocketWapper, addressRemote, portRemote, rawClientData, webSocket, amclubsResponseHeader, log, addressType);
        },
        close() {
 ...
 * @param {Uint8Array} amclubsResponseHeader The amclubs response header.
 * @param {function} log The logging function.
 * @returns {Promise<void>} The remote socket.
 */
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, amclubsResponseHeader, log, addressType,) {
    ...
    async function retry() {
        ...
        remoteSocketToWS(tcpSocket, webSocket, amclubsResponseHeader, null, log);
    }
    ...
    // remote--> ws
    remoteSocketToWS(tcpSocket, webSocket, amclubsResponseHeader, retry, log);
}
...
// https://xtls.github.io/development/protocols/amclubs.html

/**
 * Processes the amclubs header buffer and returns an object with the relevant information.
 * @param {ArrayBuffer} amclubsBuffer The amclubs header buffer to process.
 * @param {string} userID The user ID to validate against the UUID in the amclubs header.
 * @returns {{
 *  hasError: boolean,
 *  message?: string,
 ...
 *  portRemote?: number,
 *  rawDataIndex?: number,
 *  amclubsVersion?: Uint8Array,
 *  isUDP?: boolean
 * }} An object with the relevant information extracted from the amclubs header buffer.
 */
function processamclubsHeader(amclubsBuffer, userID) {
    if (amclubsBuffer.byteLength < 24) {
        return {
            hasError: true,
    ...
    const version = new Uint8Array(amclubsBuffer.slice(0, 1));
    let isValidUser = false;
    let isUDP = false;
    const slicedBuffer = new Uint8Array(amclubsBuffer.slice(1, 17));
    ...
    const optLength = new Uint8Array(amclubsBuffer.slice(17, 18))[0];
    //skip opt for now

    const command = new Uint8Array(
        amclubsBuffer.slice(18 + optLength, 18 + optLength + 1)
    )[0];
    ...
    const portBuffer = amclubsBuffer.slice(portIndex, portIndex + 2);
    // port is big-Endian in raw data etc 80 == 0x005d
    const portRemote = new DataView(portBuffer).getUint16(0);

    let addressIndex = portIndex + 2;
    const addressBuffer = new Uint8Array(
        amclubsBuffer.slice(addressIndex, addressIndex + 1)
    );
    ...
    switch (addressType) {
        case 1:
            ...
            addressValue = new Uint8Array(
                amclubsBuffer.slice(addressValueIndex, addressValueIndex + addressLength)
            ).join('.');
            break;
        case 2:
            addressLength = new Uint8Array(
                amclubsBuffer.slice(addressValueIndex, addressValueIndex + 1)
            )[0];
            addressValueIndex += 1;
 ...
 * @param {ArrayBuffer | null} amclubsResponseHeader The amclubs response header.
 * @param {(() => Promise<void>) | null} retry The function to retry the connection if it fails.
 * @param {(info: string) => void} log The logging function.
 * @returns {Promise<void>} A Promise that resolves when the conversion is complete.
 */
async function remoteSocketToWS(remoteSocket, webSocket, amclubsResponseHeader, retry, log) {
    // remote--> ws
    let remoteChunkCount = 0;
    let chunks = [];
    /** @type {ArrayBuffer | null} */
    let amclubsHeader = amclubsResponseHeader;
        ...
        .pipeTo(
            new WritableStream({
                ...
                async write(chunk, controller) {
                    ...
                    if (amclubsHeader) {
                        webSocket.send(await new Blob([amclubsHeader, chunk]).arrayBuffer());
                        amclubsHeader = null;
                    } else {
 ...
 * @param {ArrayBuffer} amclubsResponseHeader The amclubs response header.
 * @param {(string) => void} log The logging function.
 * @returns {{write: (chunk: Uint8Array) => void}} An object with a write method that accepts a Uint8Array chunk to write to the transform stream.
 */
async function handleUDPOutBound(webSocket, amclubsResponseHeader, log) {

    let isamclubsHeaderSent = false;
    ...
    transformStream.readable.pipeTo(new WritableStream({
        async write(chunk) {
            ...
            if (webSocket.readyState === WS_READY_STATE_OPEN) {
                ...
                if (isamclubsHeaderSent) {
                    webSocket.send(await new Blob([udpSizeBuffer, dnsQueryResult]).arrayBuffer());
                } else {
                    webSocket.send(await new Blob([amclubsResponseHeader, udpSizeBuffer, dnsQueryResult]).arrayBuffer());
                    isamclubsHeaderSent = true;
                }
            }
        }
    })).catch((error) => {
 ...
 * @returns {{write: (chunk: Uint8Array) => void}} An object with a write method that accepts a Uint8Array chunk to write to the transform stream.
 */
async function handleDNSQuery(udpChunk, webSocket, amclubsResponseHeader, log) {
    try {
        ...
        await tcpSocket.readable.pipeTo(new WritableStream({
            async write(chunk) {
                if (webSocket.readyState === WS_READY_STATE_OPEN) {
                    const dataToSend = amclubsHeader ? await new Blob([amclubsHeader, chunk]).arrayBuffer() : chunk;
